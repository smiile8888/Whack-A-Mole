...
Game.prototype.clickOnCell = function(event) {
    ...
    var hitElement = event.target.childNodes
};
...
Grid.prototype.generateChick = function () {
    var randomColumn = Math.floor(Math.random() * 4).toString();
    ...
    if (!~this.moles.indexOf(element)) {
        ...
        var chick = new Image();
        chick.id = "chick";
        chick.src = "../prototype/media/chick.png";
        chick.style.maxHeight = "100px";
        element.appendChild(chick);
        // element.getElementById("mole").style.display = "block";
        this.chicks.push(element);
    }
};
Grid.prototype.cleanChicks = function () {
    for(var i = 0; i < this.chicks.length; i++) {
        this.chicks[i].removeChild(document.getElementById("chick"));
        // this.moles[i].style.backgroundColor = 'rgba(0, 0, 0, 0)';
        // this.moles[i].document.getElementById("mole").style.display = "none";
    }
    this.chicks = [];
};
Grid.prototype.stopChicks = function () {
    this.cleanChicks();
};
